...
const FeedbackPage = () => {
  ...
  const inputClass = "mt-2 h-12 w-full rounded-xl border border-stone-300 bg-white px-4 text-sm text-stone-950 transition-all focus:border-orange-400 focus:ring-4 focus:ring-orange-500/10 dark:border-white/10 dark:bg-black/20 dark:text-white";
      ...
      <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        <section className="surface-card rounded-[20px] p-5 sm:p-7">
          <div className="mb-6 flex items-start gap-4 rounded-2xl bg-[#171717] p-5 text-white">
            <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-orange-500 text-white"><MessageSquareHeart size={21} /></span>
        ...
        <section className="surface-card rounded-[20px] p-5 sm:p-7">
          <div className="mb-5 flex items-center justify-between"><div><p className="eyebrow mb-2">Your voice</p><h2 className="text-xl font-extrabold text-stone-950 dark:text-white">Recent feedback</h2></div><Sparkles className="h-5 w-5 text-orange-500" /></div>
          {loadingHistory ? <div className="space-y-3">{[0, 1, 2].map((item) => <Skeleton key={item} className="h-28" />)}</div> : historyError ? <div className="rounded-xl border border-red-300/40 bg-red-50 p-4 text-sm text-red-700 dark:bg-red-500/10 dark:text-red-200">{historyError}<button type="button" onClick={loadFeedback} className="ml-2 font-bold underline">Try again</button></div> : feedback.length === 0 ? <div className="rounded-2xl border border-dashed border-stone-300 px-6 py-12 text-center dark:border-white/10"><MessageSquareHeart className="mx-auto h-9 w-9 text-orange-500" /><h3 className="mt-4 font-bold text-stone-950 dark:text-white">You haven&apos;t submitted feedback yet.</h3><p className="mt-1 text-sm text-stone-500">Your recent responses will appear here.</p></div> : (
            <div className="space-y-3">
              {feedback.map((item) => (
                <article key={item._id} className="rounded-2xl border border-stone-200 p-4 dark:border-white/10">
                  <div className="flex items-center justify-between gap-3"><span className="rounded-full bg-orange-500/10 px-2.5 py-1 text-[11px] font-bold text-orange-700 dark:text-orange-300">{item.category}</span><time className="text-[11px] text-stone-500">{new Date(item.createdAt).toLocaleDateString()}</time></div>
                  ...
                  <div className="mt-3 flex items-center justify-between"><span className="flex gap-0.5 text-orange-500">{Array.from({ length: item.rating }, (_, index) => <Star key={index} size={12} fill="currentColor" />)}</span><span className="text-[10px] font-bold uppercase tracking-wide text-stone-500">{item.status}</span></div>
                </article>
              ))}
            </div>
          )}
