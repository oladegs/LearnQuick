...
const AboutPage = () => (
  <div>
    ...
    <section className="relative overflow-hidden rounded-[24px] bg-[#171717] p-7 text-white shadow-2xl shadow-stone-950/15 sm:p-10">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_86%_20%,rgba(37,99,235,0.35),transparent_23rem)]" />
      <div className="relative grid gap-8 lg:grid-cols-[1fr_0.45fr] lg:items-end">
        <div className="max-w-3xl">
          <span className="mb-6 flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-500 shadow-lg shadow-blue-500/25"><BrainCircuit size={28} strokeWidth={2.4} /></span>
        ...
        <div className="rounded-2xl border border-blue-400/20 bg-blue-500/10 p-5">
          <Sparkles className="mb-3 h-6 w-6 text-blue-300" />
          <p className="text-sm font-bold leading-6 text-blue-100">One connected flow for reading, asking, remembering, and practicing.</p>
        </div>
      </div>
    </section>
    ...
    <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      {features.map((feature) => (
        <article key={feature.title} className="surface-card group rounded-[18px] p-6 transition-all duration-200 hover:-translate-y-1 hover:border-blue-300 dark:hover:border-blue-400/30">
          <span className="mb-5 flex h-11 w-11 items-center justify-center rounded-xl bg-blue-500/10 text-blue-600 group-hover:bg-blue-500 group-hover:text-white dark:text-blue-300"><feature.icon size={21} strokeWidth={2.3} /></span>
        ...
        {steps.map((step, index) => (
          <div key={step} className="flex items-center gap-3 sm:block">
            <div className="flex items-center gap-3">
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-blue-500 text-sm font-extrabold text-white">{index + 1}</span>
